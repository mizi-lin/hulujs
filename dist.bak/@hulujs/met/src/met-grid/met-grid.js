...
const MetGrid = (props) => {
    const { children, inline, cols, rows, className = '', align, alignItems = align, justify, justifyContent = justify, ...extra } = props;
    const attr = { ...adjustStyle(cols, 'Columns'), ...adjustStyle(rows, 'Rows') };
    return (_jsx(Met, { display: "grid", className: clx('met-grid', className), inline: inline, ...compact({ alignItems, justifyContent }), ...attr, ...extra, children: children }));
};
