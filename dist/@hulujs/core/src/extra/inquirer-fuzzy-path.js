    ...
    async function listNodes(nodePath, level) {
        try {
            ...
            if (nodes.length > 0 && (depthLimit === undefined || level >= 0)) {
                const nodesWithPath = nodes.map((nodeName) => listNodes(path.join(nodePath, nodeName), depthLimit ? level - 1 : undefined));
...
class InquirerFuzzyPath extends InquirerAutocomplete {
    constructor(question, rl, answers) {
        ...
        const questionBase = Object.assign({}, question, {
            source: (_, pattern) => getPaths(rootPath, pattern, excludePath, excludeFilter, itemType, question.default, depthLimit)
        });
