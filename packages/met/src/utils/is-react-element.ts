 ...
 */
const isHTMLElement = (element: ReactNode | ElementType) => {
    if (typeof element === 'string') return isValidElementType(element);
    if (typeof element?.['type'] === 'string') return isValidElementType(element?.['type']);
    return false;
};
