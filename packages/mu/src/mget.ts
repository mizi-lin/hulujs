 ...
 * 字符串与图标互换，防止配置过程中污染
 */
const replaceSign = (str: string | number, toggle = false) => {
 ...
 * @param p1
 */
const instead = (matcher: string, p1: string) => {
...
export const propPathToCash = (path: PropPaths) => {
    /**
     * 将下列的几种情况转为
     * - a."b.c.d".[dd]c
     * - a.["b.c.d"]."[dd]".c
...
const baseGet = (
    ...
    if (cash.includes('*')) {
        ...
        // 如果不是对象，则返回void
        // 如果是对象，则map取值
        const aData = baseGet(obj, a, prevCash) as BaseGetResult;
        if (!isObject(aData?.value)) return { value: void 0, cash: a };
    ...
    return cash.reduce(
        ({ value, cash: cash$ }, key: any) => {
            const cash = [...cash$, key];
            if (isNil(value)) return { value, cash };
            return { value: value[replaceSign(key, true)], cash };
        },
        { value: obj, cash: prevCash } as BaseGetResult
    );
};
 ...
 * @returns
 */
const mget = (obj: Record<string, any>, path: PropPaths, type: MgetType = 'normal'): any => {
    const result = baseGet(obj, path, []);
    if (type === 'detail') return result;
