...
function reargs(args) {
    ...
    const type =
        typeof extra[0] === 'string' && ['normal', 'decimal', 'percent', 'toPercent', 'currency'].includes(extra[0]) ? extra[0] : 'normal';
...
// 千分位处理
function thousands(num: string, count = 3, delimiter = ','): string {
    const reg = new RegExp(`(\\d{1,${count}})(?=(?:\\d{${count}})+$)`, 'g');
    return num.replace(reg, `$1${delimiter}`);
}

function numberFormat(value: number, options?: Partial<NumberFormatOptions>);
function numberFormat(value: number, precision?: number, options?: Partial<NumberFormatOptions>);
