...
import { flatten, isNil, isPlainObject } from 'lodash-es';
 ...
 */
const isEntriesItem = (entries: any) => {
...
// -> [[a, 1], [b, 2], [c, 3], [d, 4], [e, 5]]
const parisToEntries = (paris: any = {}, value?: any) => {
    if (isNil(paris)) return [];
    if (!isNil(value)) return [[cashToPropPath(paris), value]];
    // 当值是对象的时候, 直接转为键值对
    if (isPlainObject(paris)) return Object.entries(paris);
    // 直接是键值对，则返回键值对集合
    if (isEntriesItem(paris)) return [paris];
    if (Array.isArray(paris)) {
        return paris.reduce((temp, item) => {
            if (isPlainObject(item)) return [...temp, ...Object.entries(item)];
            if (isEntriesItem(item)) return [...temp, item];
