...
class Git {
     ...
     */
    async acp(commitMsg: string, addFiles: string | string[] = '.') {
        const status = await this.status();
        const { isChange, current, tracking } = status;

        if (!isChange) return void 0;
    ...
    async pull(params: string[]) {
        ...
        try {
            ...
            if ('HEAD' === current) return void 0;
            if (!isNil(tracking)) await this.git.branch(['--unset-upstream']);
            await this.tracking();
    ...
    async tracking() {
        const { tracking, current } = await this.status();
        if ('HEAD' === current) return void 0;
        if (!isNil(tracking)) return void 0;
        const remote = await this.getRemote();
        try {
            // 远程拥有这个分支
